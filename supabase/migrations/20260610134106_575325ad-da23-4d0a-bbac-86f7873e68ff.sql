...
-- Harden: remove overly permissive INSERT policies (place_order is SECURITY DEFINER and bypasses RLS)
DROP POLICY IF EXISTS "Anyone can create orders" ON public.orders;
DROP POLICY IF EXISTS "Anyone can insert order items" ON public.order_items;

-- Lock down SECURITY DEFINER functions: revoke from PUBLIC, grant only to intended roles
...
-- place_order intentionally remains callable by anon (guest checkout); revoke from PUBLIC and grant explicitly
REVOKE EXECUTE ON FUNCTION public.place_order(text, text, text, text, text, jsonb, text, text, text, text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.place_order(text, text, text, text, text, jsonb, text, text, text, text, text, text, text) TO anon, authenticated;

-- Harden place_order: input length limits and basic shape validation to prevent abuse
CREATE OR REPLACE FUNCTION public.place_order(p_customer_name text, p_customer_email text, p_customer_phone text, p_payment_method text, p_delivery_method text, p_items jsonb, p_zip text DEFAULT NULL::text, p_street text DEFAULT NULL::text, p_number text DEFAULT NULL::text, p_complement text DEFAULT NULL::text, p_district text DEFAULT NULL::text, p_city text DEFAULT NULL::text, p_state text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  VALUES (
    auth.uid(), trim(p_customer_name), lower(trim(p_customer_email)), p_customer_phone,
