...
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS manual_sale boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS manual_created_by uuid REFERENCES auth.users(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS manual_created_by_name text;
...
REVOKE EXECUTE ON FUNCTION public.create_manual_order(text, text, text, jsonb, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.create_manual_order(text, text, text, jsonb, text, text) TO authenticated;
