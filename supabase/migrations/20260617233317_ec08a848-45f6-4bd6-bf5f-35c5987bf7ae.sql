...
-- =====================================================================
-- 1) Reserva atômica de estoque no pedido pendente
-- =====================================================================
...
BEGIN
  ...
  -- Calcula total e valida (com FOR UPDATE bloqueando linha de produto)
  ...
  -- Cria pedido pendente com stock_restored_at = NULL (estoque RESERVADO)
  ...
  -- Insere itens E reserva estoque atomicamente
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    v_qty := (v_item->>'quantity')::int;
    v_color := nullif(trim(coalesce(v_item->>'color', '')), '');
...
-- =====================================================================
-- 2) confirm_order_paid: estoque JÁ foi reservado, apenas confirmar.
--    Se stock_restored_at NÃO é NULL, então (legado/expirado) re-reservar atômico.
-- =====================================================================
...
BEGIN
  ...
  -- Se estoque NÃO está reservado (legado ou expirado), tentar reservar agora
...
-- =====================================================================
-- 3) Tabela de histórico de reembolsos (snapshot p/ reimprimir comprovante)
-- =====================================================================
...
CREATE POLICY "Admins veem reembolsos"
  ON public.refunds FOR SELECT
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins inserem reembolsos"
